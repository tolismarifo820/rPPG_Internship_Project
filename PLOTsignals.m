...
% basePath = 'C:\Users\tolis\Desktop\Praktiki\rPPG_project\logs\P001\';
% 
% % List of all files to extract signals from
% fileNames = {'raw_signals.csv', 'rppg_raw_ml.csv', 'summary.csv'};
% 
% % Initialize cell arrays to hold signal data and their labels
% allSignals = {};
% allTitles = {};
% 
% % 1. Read all files and extract every numeric signal
% for i = 1:length(fileNames)
%     fullPath = fullfile(basePath, fileNames{i});
% 
%     if isfile(fullPath)
...
%             fileNameClean = strrep(fileNames{i}, '_', '\_');
...
% end
% 
% % 2. Plot the collected signals in chunks of 3
...
%         figure('Name', sprintf('Combined Signals - Part %d', figIdx), ...
...
% 
%         % Main figure title
%         sgtitle(sprintf('rPPG and Vital Signals (Plots %d to %d of %d)', ...
%             startIdx, endIdx, numSignals), 'FontSize', 14, 'FontWeight', 'bold');
%     end
% else
%     disp('No numeric signal data found in any of the specified files.');
% end
