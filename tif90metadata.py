...
def draw_camera_card(canvas, target_box, frame, evm_roi_bbox=None, mask_contours=None, is_recording=False, remaining_sec=20, participant_id="", evm_mode="BGR"):
    x, y, w, h = target_box
    cv2.rectangle(canvas, (x, y), (x + w, y + h), CARD_COLOR, -1)
    ...
    try:
        ...
        while True:
            ...
            if RUN_ON_LAPTOP:
                ...
                try: frame = cv2.cvtColor(picam2.capture_array(), cv2.COLOR_RGB2BGR)
            ...
            if vitals_roi is not None and vitals_mask is not None:
                ...
                processed_roi = cv2.merge(bgr_to_yiq(vitals_roi)) if active_evm_mode == "YIQ" else vitals_roi.copy().astype(np.float32)
                ...
                laplacian = cv2.subtract(gp[target_level], cv2.pyrUp(gp[target_level + 1], dstsize=(gp[target_level].shape[1], gp[target_level].shape[0])))
                ...
                if not video_pyr_initialized:
                    ...
                    if laplacian.shape[:2] != videoPyramid.shape[1:3]: 
                        laplacian = cv2.resize(laplacian, (videoPyramid.shape[2], videoPyramid.shape[1]))

                videoPyramid[bufferIndex] = laplacian
                fft_buffer = np.fft.fft(np.roll(videoPyramid, -bufferIndex - 1, axis=0), axis=0)
                fft_buffer[~mask] = 0  
                
                filtered_laplacian = np.real(np.fft.ifft(fft_buffer, axis=0))[-1] * (alpha_yiq if active_evm_mode == "YIQ" else alpha_bgr)
                ...
                masked_pulse = np.clip(cv2.resize(amplified_signal, (vitals_roi.shape[1], vitals_roi.shape[0])) * np.expand_dims((vitals_mask > 0).astype(np.float32), axis=-1), -15.0, 15.0)
            ...
            if buffers_initialized and face_detected_now:
                ...
                active_signal = bandpass_filter(raw_extracted_signal, hr_low, hr_high, fps, order=2) if vitals_enabled else raw_extracted_signal

                if vitals_enabled and bufferIndex % bpmCalcEvery == 0:
                    bpm = estimate_peak_bpm(active_signal, fps, hr_low, hr_high)
                    if len(bpm_all) > 0: 
                        bpm = float(np.clip(bpm, bpm_all[-1] - 3.0 * (bpmCalcEvery / fps), bpm_all[-1] + 3.0 * (bpmCalcEvery / fps)))
                    ...
                    rr_raw = estimate_peak_bpm(bandpass_filter(r_rolled, rr_low, rr_high, fps, order=2), fps, rr_low, rr_high)
                    if 6 <= rr_raw <= 40: 
                    ...
                    if dc_r > 1e-3 and dc_b > 1e-3 and ac_b > 1e-6:
                        ...
                        spo2_raw = min(float(SPO2_A - (SPO2_B * ratio_of_ratios)), 100.0)

                        if 85.0 <= spo2_raw <= 100.0:
                            spo2_history.append(spo2_raw)

                    spo2_smoothed = robust_mean(spo2_history)
                    current_spo2 = spo2_smoothed if spo2_smoothed is not None else None
            ...
            if is_recording:
                if SAVE_CAMERA_ACQUISITION and video_writer is not None: 
            ...
            draw_camera_card(canvas, BOX_CAMERA, frame, evm_roi_bbox=evm_roi_bbox, mask_contours=mask_contours, is_recording=is_recording, remaining_sec=remaining_sec, participant_id=current_participant_id, evm_mode=active_evm_mode)
            ...
            if key == ord('q'): 
                ...
                if not is_recording:
                    ...
                    if SAVE_CAMERA_ACQUISITION:
                        video_writer = open_acquisition_video_writer(sess_dir, raw_frame_for_video, fps)
            elif key == ord('n'):
                if is_recording:
                    is_recording = False
                    if video_writer is not None:
